...
如果没有找到合适的匹配，返回：
{
  ...
  static load(env = {}) {
    ...
    const envVarConfig = {
      ...
      'AUTO_MATCH_MAPPING_TABLE_URL': { category: 'match', type: 'text', description: '远程季集转换表，默认关闭。支持 GitHub 文件页、Gist、jsDelivr 和 TXT 直链；下载后缓存在本机，每天北京时间 05:30 更新，失败时沿用旧缓存。本机 AUTO_MATCH_MAPPING_TABLE 优先于远程规则。' },
