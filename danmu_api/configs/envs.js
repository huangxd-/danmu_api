...
如果没有找到合适的匹配，返回：
{
  ...
  static load(env = {}) {
    ...
    const envVarConfig = {
      ...
      'AUTO_MATCH_MAPPING_TABLE_URL': { category: 'match', type: 'text', description: '远程季集转换表，默认关闭；本机 AUTO_MATCH_MAPPING_TABLE 优先。\nNode/Docker：下载到 .cache，每天北京时间 05:30 更新，失败时使用磁盘缓存。\nServerless：每个冷启动实例首次匹配时下载，只保存在当前实例内存中；不保证磁盘缓存、定时更新或跨实例手动刷新。' },
    ...
    return {
      ...
      autoMatchMappingTableUrl: this.get('AUTO_MATCH_MAPPING_TABLE_URL', '', 'string'), // 远程季集转换表地址
