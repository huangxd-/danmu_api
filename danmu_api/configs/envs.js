...
如果没有找到合适的匹配，返回：
{
  ...
  static load(env = {}) {
    ...
    const envVarConfig = {
      ...
      'TITLE_MAPPING_TABLE': { category: 'match', type: 'map', description: '剧名映射表，用于自动匹配时替换标题进行搜索，格式：原始标题->映射标题;原始标题->映射标题;... ，例如："唐朝诡事录之西行->唐朝诡事录;锦绣芳华->国色芳华"' },
