...
如果没有找到合适的匹配，返回：
{
  ...
  static load(env = {}) {
    ...
    const envVarConfig = {
      ...
      'GRADIENT_CHANCE': { category: 'danmu', type: 'number', min: 0, max: 100, description: '渐变色弹幕概率（CONVERT_COLOR为color时生效），单位百分比 0-100，默认 5，弹幕按此概率从渐变色带取色（随出现时间平滑流转），0 表示关闭' },
    ...
    return {
      ...
      gradientChance: this.get('GRADIENT_CHANCE', 5, 'number'), // 渐变色弹幕出现概率（百分比），CONVERT_COLOR为color时生效，0 表示关闭
