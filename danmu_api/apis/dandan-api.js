...
import CustonSource from "../sources/misaka.js";
...
const custonSource = new CustonSource();
...
export async function searchAnime(url, preferAnimeId = null, preferSource = null) {
  ...
  try {
    ...
    const requestPromises = globals.sourceOrderArr.map(source => {
      ...
      if (source === "Custon Source") return custonSource.search(queryTitle);
    ...
    const {
      ...
      hanjutv: animesHanjutv, bahamut: animesBahamut, dandan: animesDandan, "Custon Source": animesCustonSource, tencent: animesTencent, youku: animesYouku,
    ...
    for (const key of globals.sourceOrderArr) {
      ...
      } else if (key === 'Custon Source') {
        // 等待处理Custon Source来源
        await custonSource.handleAnimes(animesCustonSource, queryTitle, curAnimes);
      } else if (key === 'tencent') {
        // 等待处理Tencent来源
...
export async function getComment(path, queryFormat, segmentFlag) {
  ...
  if (!urlPattern.test(url)) {
    ...
    } else if (plat === "Custon Source") {
      danmus = await custonSource.getComments(url, plat, segmentFlag);
    }
  }
...
export async function getSegmentComment(segment, queryFormat) {
  try {
    ...
    } else if (platform === "Custon Source") {
      danmus = await custonSource.getSegmentComments(segment);
