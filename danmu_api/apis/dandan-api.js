...
import MisakaSource from "../sources/misaka.js";
...
const misakaSource = new MisakaSource();
...
export async function searchAnime(url, preferAnimeId = null, preferSource = null) {
  ...
  try {
    ...
    const requestPromises = globals.sourceOrderArr.map(source => {
      ...
      if (source === "misaka") return misakaSource.search(queryTitle);
    ...
    const {
      ...
      hanjutv: animesHanjutv, bahamut: animesBahamut, dandan: animesDandan, misaka: animesMisaka, tencent: animesTencent, youku: animesYouku,
    ...
    for (const key of globals.sourceOrderArr) {
      if (key === '360') {
        ...
        // 等待处理Misaka来源
        await misakaSource.handleAnimes(animesMisaka, queryTitle, curAnimes);
      } else if (key === 'tencent') {
        // 等待处理Tencent来源
...
export async function getComment(path, queryFormat, segmentFlag) {
  ...
  if (!urlPattern.test(url)) {
    if (plat === "renren") {
      ...
      danmus = await misakaSource.getComments(url, plat, segmentFlag);
    }
  }
...
export async function getSegmentComment(segment, queryFormat) {
  try {
    ...
    } else if (platform === "misaka") {
      danmus = await misakaSource.getSegmentComments(segment);
