...
上传时需要填写标题、年份和类型（`tv` 或 `movie`）。电视剧默认第 1 季第 1 集，也可以指定季数和集数；电影的季数和集数可以留空。标题、年份、类型和季相同的资源会归为一组，列表支持按标题关键词搜索，可展开查看每一集并单独删除文件、重新上传文件，也可以一次删除整个剧集。重新上传同一季同一集会替换原文件。
