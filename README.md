...
| AUTO_MATCH_MAPPING_TABLE_URL | 【可选】远程季集转换表地址，默认关闭。支持 GitHub 文件页、Gist、jsDelivr 和 TXT 直链；首次使用下载并缓存到 `.cache`，每天北京时间 05:30 自动更新，失败时沿用旧缓存。本机 `AUTO_MATCH_MAPPING_TABLE` 与远程规则同时命中时优先使用本机规则。可携带有效 TOKEN 调用 `POST /api/auto-match-mapping/refresh` 手动刷新。       |
