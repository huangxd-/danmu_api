...
| BLOCKED_WORDS    | 【可选】弹幕屏蔽词列表，默认为空，多条规则用逗号分隔（中英文逗号均可，逗号前后空格会自动忽略）。每条规则支持四种写法：正则 `/正则/` 或 `/正则/i`（可带 i/m/s/u 等标志）、纯文本词（按字面匹配）、`@人名`（按语境分析匹配：三字及以上按完整名称匹配；二字人名仅在明确人物语境中命中，如"喜欢白鹿""白鹿老师""@白鹿"，不会误伤"白鹿原"；姓氏仅在"姓氏+称谓"或"@姓氏"显式指代时命中）、`地区:地区名`（仅命中"来自海南""海南网友""朝阳区"等明确地区语境，不会误伤"海南鸡饭"；地区名由用户自行填写，无内置地区名单）。示例如下       |
