...
| BLOCKED_WORDS    | 【可选】弹幕屏蔽词列表，默认为空，多条规则用逗号分隔（中英文逗号均可，逗号前后空格会自动忽略）。每条规则支持四种写法：正则 `/正则/` 或 `/正则/i`（可带 i/m/s/u 等标志）、纯文本词（按字面匹配）、`@人名`（按语境分析匹配：三字及以上按完整名称匹配；二字人名仅在明确人物语境中命中，如"喜欢白鹿""白鹿老师""@白鹿"，不会误伤"白鹿原"；姓氏仅在"姓氏+称谓"或"@姓氏"显式指代时命中）、`地区:地区名`（仅命中"来自海南""海南网友""朝阳区"等明确地区语境，不会误伤"海南鸡饭"；地区名由用户自行填写，`地区:*` 或 `地区:全部` 启用内置预设地区名单：中国大陆省级行政区+地级行政区）。示例如下       |
| BLOCK_DOMESTIC_CELEBRITIES | 【可选】当前华语作品演员/角色名屏蔽开关，默认 `false`。开启后通过 TMDB 获取当前国产/港台作品的中文演员名、角色名及明确称谓/身份的姓氏指代（如“王老师”“欧阳导演”）；二字名称仅在明确人物语境中匹配，三字及以上按完整名称匹配。需要可用的 `TMDB_API_KEY`，或能够代为认证的 TMDB 反代；查询失败时不执行该项过滤       |
...
| TMDB_API_KEY    | 【可选】TMDB API Key，用于直连 TMDB 的标题转换、巴哈译名辅助及当前作品演员/角色表。未配置时仍可使用能够代为认证的 TMDB 反代；两者都不可用时，当前作品演员/角色过滤不会生效；[TMDB API Key](https://www.themoviedb.org/settings/api)获取方法参考：[TMDB API Key申请 - 绿联NAS私有云](https://www.ugnas.com/tutorial-detail/id-226.html)       |
