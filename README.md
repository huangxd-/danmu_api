...
| AUTO_MATCH_MAPPING_TABLE    | 【可选】自动匹配映射表，仅作用于 `POST /api/v2/match`，多条规则用分号分隔。映射表是可选纠错能力，不配置任何剧名或季集映射时，自动匹配仍会独立完成 Emby 文件名解析、标题/别名校验、季度和真实集号选择。文件名带年份时同年候选优先、无年份候选次之；第一季和电影会排除明确异年结果，第二季及以后在标题与季度一致时允许源站“当前季首播年”兜底。文件名不带年份时不判断年份。映射目标未写年份时继续使用文件名年份，目标标题显式写了年份时以目标年份为准。开放映射 `永生 S05E02 -> 永生 S01E58` 会在源第 5 季内按集数递增映射；同标题同季度可配置多个开放规则，后面起始集数的规则会从该集开始覆盖前面的规则，例如 `一念永恒 S01E53 -> 一念永恒 S02E01;一念永恒 S01E107 -> 一念永恒 S03E01`；有限范围 `永生 S05E02~03 -> 永生 S01E58~59` 只映射包含两端的等长范围。支持目标结果优选 `海贼王 S02E01 -> 航海王(1999)【动漫】 S01E62` 和平台优选 `航海王 S01E01 -> 航海王 S01E01 @qiyi`。剧名表与季集表可组合使用：先尝试原始标题的季集规则，未命中时用 `TITLE_MAPPING_TABLE` 转成标准标题并再次查季集规则。整体优先级为当前源季手动偏好 > 原始标题季集规则 > 标准标题季集规则 > 剧名映射后的普通匹配；没有剧名映射时直接使用原始标题普通匹配，`default` 偏好不阻断映射。限定候选不可用时回退同目标标题，映射目标失败时回到上述普通匹配链路。普通搜索、收藏缓存和弹幕时间偏移不受影响。       |
