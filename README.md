...
| VOD_SERVER2      | 【可选】vod2查询站点，如果想开启vod2源，请填写源地址，如`https://zy.xmm.hk`，并在SOURCE_ORDER环境变量中增加`vod2`       |
...
| SOURCE_ORDER    | 【可选】源排序，用于按源对返回资源的排序（注意：先后顺序会影响自动匹配最终的返回），默认是`vod,360,renren,hanjutv,bahamut`，表示vod数据排在最前，renren数据排在最后，示例：`360,renren`：只返回360数据和renren数据，且360数据靠前       |
...
| BAHAMUT_PROXY    | 【可选】巴哈姆特反向代理，（注意：默认不开启bahamut源，请先在SOURCE_ORDER环境变量中添加`bahamut`）如果你在docker或其他平台部署并且访问不了bahamut，请配置反向代理，部署请参考       |
