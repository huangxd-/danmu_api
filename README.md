...
- **API 接口**：
  ...
  - `POST /api/v2/match`：根据关键字匹配动漫，用于自动匹配。（已支持在match接口中通过@语法动态指定平台优先级，如`赴山海 S01E28 @qiyi`；已支持从网盘资源命名，如`无忧渡.S01E01.2160p.WEB-DL.H265.DDP.5.1`中提取 title/season/episode；`S00E01`与`S0E01`按特别篇匹配，只接受标题明确带`特别篇`、`番外`、`OVA`、`OAD`、`SP`或`Special`的候选，不会混入普通第一季）；可通过 `AUTO_MATCH_MAPPING_TABLE` 配置跨标题、跨季和集数范围映射；已支持外语标题匹配，如`Blood.River.S01E05`，需配置环境变量`TITLE_TO_CHINESE`使用；已适配该格式`爱情公寓.ipartment.2009.S03E05.H.265.25fps.mkv`标题；已支持AI自动匹配，需配合AI相关环境变量使用
...
| AUTO_MATCH_MAPPING_TABLE    | 【可选】自动匹配映射表，仅作用于 `POST /api/v2/match`，多条规则用分号分隔。映射表只用于兜底：系统先按原始 Emby 文件名执行本机普通自动匹配，成功后直接返回，不再让剧名表或季集表覆盖结果；只有本机匹配失败时，才依次尝试原始标题季集规则、剧名映射后的标准标题季集规则和剧名映射普通匹配。不配置任何映射时，本机匹配仍会独立完成文件名解析、标题/别名校验、季度和真实集号选择。文件名带年份时同年候选优先、无年份候选次之；第一季和电影会排除明确异年结果，第二季及以后在标题与季度一致时允许源站“当前季首播年”兜底。文件名不带年份时完全不使用年份作为匹配条件。映射目标未写年份时继续使用文件名年份，目标标题显式写了年份时以目标年份为准。开放映射 `永生 S05E02 -> 永生 S01E58` 会在源第 5 季内按集数递增映射；同标题同季度可配置多个开放规则，后面起始集数的规则会从该集开始覆盖前面的规则，例如 `一念永恒 S01E53 -> 一念永恒 S02E01;一念永恒 S01E107 -> 一念永恒 S03E01`；有限范围 `永生 S05E02~03 -> 永生 S01E58~59` 只映射包含两端的等长范围。支持目标结果优选 `海贼王 S02E01 -> 航海王(1999)【动漫】 S01E62` 和平台优选 `航海王 S01E01 -> 航海王 S01E01 @qiyi`。整体优先级为本机普通自动匹配（含当前源季手动偏好）> `AUTO_MATCH_MAPPING_TABLE` > `TITLE_MAPPING_TABLE`。限定候选不可用时回退同目标标题。普通搜索、收藏缓存和弹幕时间偏移不受影响。       |
