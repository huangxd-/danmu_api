...
| UI_THEME    | 【可选】管理界面默认主题，默认为 `shinyo`（新叶绿）。浏览器中选择的主题会保存在本地并优先使用。可选值：`shinyo`（新叶绿）、`sakura`（哔哩粉）、`tianyi`（天依蓝）、`hatsune`（初音青）、`sakuragi`（樱木红）、`violet`（罗兰紫）、`amber`（LCL橘）、`lavender`（经典默认）       |
