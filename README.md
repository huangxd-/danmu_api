...
| AUTO_MATCH_MAPPING_TABLE_URL | 【可选】远程季集转换表地址，默认关闭。支持 GitHub 文件页、Gist、jsDelivr 和 TXT 直链，本机 `AUTO_MATCH_MAPPING_TABLE` 优先于远程规则。Node/Docker：首次使用时下载到 `.cache`，每天北京时间 05:30 自动更新，下载失败时继续使用磁盘缓存；携带有效 TOKEN 调用 `POST /api/auto-match-mapping/refresh` 可立即下载并更新缓存。Serverless：每个冷启动实例在首次匹配时下载规则，仅缓存在该实例内存中；`.cache` 不持久化，05:30 定时更新不能保证执行，手动刷新也只影响收到请求的当前实例。       |
